...
" GT_ZEDT004 테이블 ZCODE ZPERNR 기준으로 정렬
...
LOOP AT GT_ZEDT004 INTO GS_ZEDT004.
  ...
  " 루프마다 GS_ZEDT001, GS_ZEDT002 초기화
  CLEAR : GS_ZEDT001, GS_ZEDT002.

  " GS_ZEDT004의 ZCODE에 맞는 학생 정보 읽기
  ...
  " GS_ZEDT001에 해당 학생 코드가 없으면 CONTINUE
  IF SY-SUBRC <> 0.
    CONTINUE.
  ENDIF.
  ...
  " 첫 ZCODE인 경우 전과 여부, 학사 경고 여부, 최저 학점 플래그 초기화
  AT NEW ZCODE.
    GV_ZTRANSFER = 'T'.
    GV_ZGRADE = 'T'.
    GV_ZLOWGRADE = 'A'.
  ...
  " 학점이 최저 학점 변수의 값보다 낮으면 내부 값 최신화
  ...
  AT END OF ZCODE.
    ...
    " 동일 키(ZGENDER)가 이미 존재하면 ZSUM 자동 합산
    ...
    IF GV_ZTRANSFER <> 'T'.
      " 학사 경고 대상이 아닌 경우
      IF GV_ZGRADE = 'T'.
        WRITE :/ ' | ', GS_ZEDT001-ZCODE, ' | ', GS_ZEDT001-ZKNAME, ' | ', '전과대상', ' | ', '                ', ' | ', '         | '.
      " 학사 경고 대상인 경우
      ELSE.
        WRITE :/ ' | ', GS_ZEDT001-ZCODE, ' | ', GS_ZEDT001-ZKNAME, ' | ', '전과대상', ' | ', GS_ZEDT001-ZTEL, ' | ', '학사경고 | '.
      ENDIF.
    " 전과 대상이 아닌 경우
    ELSE.
      " 학사 경고 대상이 아닌 경우
      ...
      " 학사 경고 대상인 경우
      ELSE.
         WRITE :/ ' | ', GS_ZEDT001-ZCODE, ' | ', GS_ZEDT001-ZKNAME, ' | ', '        ', ' | ', GS_ZEDT001-ZTEL, ' | ', '학사경고 | '.
      ENDIF.
    ENDIF.
  ...
  AT LAST.
    ...
    " GT_SUM 테이블에 저장된 값을 토대로 남학생, 여학생 총 금액 출력
    LOOP AT GT_SUM INTO GS_SUM.
      IF GS_SUM-ZGENDER = 'M'.
        WRITE: / '남학생총금액 :', GS_SUM-ZSUM.
      ELSEIF GS_SUM-ZGENDER = 'F'.
        WRITE: / '여학생총금액 :', GS_SUM-ZSUM.
      ENDIF.
    ENDLOOP.
...
WRITE :/ 'INDEX 합계: ', LV_SUM.""",
    r"""DATA : LV_NUM1 TYPE I.
DATA : LV_SUM1 TYPE I.
...
WRITE :/ 'INDEX합계:', LV_SUM1.""",
    r"""DATA : LV_NUM3 TYPE I.
DATA : LV_SUM3 TYPE I.
...
WRITE :/ 'INDEX합계: ', LV_SUM4.""",
    r"""DATA : LV_NUM2 TYPE I.
DATA : LV_SUM2 TYPE I.
...
LOOP AT GT_ZEDT004 INTO GS_ZEDT004.
  ...
  IF SY-SUBRC = 2.
    GS_WRITE-ZMAJOR_TRANSPORT = '  전과대상'. "입학당시 학과와 성적 학과가 다를경우
  ENDIF.
  ...
  ELSE.
    ...
    IF GS_ZEDT004-ZGRADE = 'D' OR GS_ZEDT004-ZGRADE ='F'. "대표성적이 D또는 F일경우 학사경고처리
      GS_WRITE-ZGRADE_MEMO = '학사경고'.
    ELSE.
      CLEAR GS_WRITE-ZTEL.
    ENDIF.
  ENDIF.
